...
class Enterprise_Rma_Block_Adminhtml_Rma_Create_Header extends Enterprise_Rma_Block_Adminhtml_Rma_Create_Abstract
{
    protected function _toHtml()
    {
        ...
        if ($customerId && $storeId) {
            ...
            $out .= Mage::helper('enterprise_rma')->__('Create New RMA for %s in %s', $customerName, $storeName);
        } elseif ($customerId) {
            $out.= Mage::helper('enterprise_rma')->__('Create New RMA for %s', $this->getCustomerName());
        } else {
            $out.= Mage::helper('enterprise_rma')->__('Create New RMA');
        }
