...
/**
 * Assert that after adding products by sku to order, requested quantity is less than allowed error message appears.
 */
class AssertMinRequestedQtyBackendFailMessage extends AbstractConstraint
{
    /* tags */
    const SEVERITY = 'low';
    /* end tags */
     ...
     */
    const ERROR_MESSAGE = 'The minimum quantity allowed for purchase is %d.';
    ...
    public function processAssert(SalesOrderCreateIndex $salesOrderCreateIndex, array $requiredAttentionProducts)
    {
        foreach ($requiredAttentionProducts as $product) {
            \PHPUnit_Framework_Assert::assertEquals(
                sprintf(self::ERROR_MESSAGE, $product->getStockData()['min_sale_qty']),
