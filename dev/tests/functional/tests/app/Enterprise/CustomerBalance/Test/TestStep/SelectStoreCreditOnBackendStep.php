...
/**
 * Select store credit on new order page.
 */
class SelectStoreCreditOnBackendStep implements TestStepInterface
{
    /**
    ...
    public function run()
    {
        if (isset($this->payment['use_customer_balance'])) {
            $this->salesOrderCreateIndex->getStoreCreditBlock()
                ->fillStoreCredit($this->payment['use_customer_balance']);
        }
    }
}
